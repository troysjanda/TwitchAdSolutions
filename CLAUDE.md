...
Bump `@version` (userscript header) and `ourTwitchAdSolutionsVersion` together for functional changes. Current: vaft 59.1.0/49, video-swap-new 1.74/42, strip 1.9/26. Testing: vaft 606.0.0/606, video-swap-new 606/606.
...
- `twitchAdSolutions_hideAdOverlay` — `true` to hide banner/promo/SDA overlays, default not set
...
- **Early reload** — fires during prolonged all-stripped freeze. Threshold: 5 polls (~10s), or 1 poll when recovery cache <3 segments (thin-cache fast path). Budget: `max(1, PodLength)` or `max(2, PodLength)` when thin. `EarlyReloadTriggered` resets on "still ads" to allow budget-based re-fire.
...
## Ad Overlay Hiding (PR #68)

`hideTwitchAdOverlays()` hides three overlay types during ad blocking:
1. Turbo promo / "allow ads" overlays — via `a[href]` selector, walk up to `.player-overlay-background`
2. Stream display ads (SDA) — via `[data-test-selector="sda-wrapper"]`
3. Ad break cards ("taking an ad break / stick around") — via text matching, one-shot logs per type

Enabled via `localStorage.twitchAdSolutions_hideAdOverlay = 'true'`. Called on every buffer monitor tick.
