...
Bump `@version` (userscript header) and `ourTwitchAdSolutionsVersion` together for functional changes. Current: vaft 67.3.0/80, video-swap-new 1.86/54, strip 1.9/26. Testing: vaft 647.0.0/647, video-swap-new -/619.
...
- `twitchAdSolutions_disableAdSpoofing` — opt-out via `'true'` (default off — spoofing is on). On ad detect, vaft fires the GQL ad-tracking beacons (`video_ad_impression`, `video_ad_quartile_complete` × 4, `video_ad_pod_complete`) that Twitch's player would have sent if the ad had played normally. Mimics the ad-completion signal Twitch expects, may reduce detection escalation. Failures swallowed (never blocks ad-block flow). Set to `'true'` to disable spoofing (for A/B testing whether spoofing affects ad break duration or detection behavior). vaft only.
